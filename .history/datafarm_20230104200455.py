...
class DataFarm():
    ...
    def __init__(self, corpus_name):
        self.corpus = self.set_corpus(corpus_name)
    ...
    def set_corpus(self, corpus_input):
        ...
        for spkr in corpus.iter_speakers:
            print(spkr)
    ...
    def create_grp_df(self, spkr_df):
        """
        ...
        :return: dataframe 
        """ 
        grp_df = self.extract_grp_ps_feats()
        grp_df = pd.concat([grp_df, self.extract_grp_coord_feats(spkr_df)], axis=1)
        # 3rd call here adding model predictions for grp sat -- just one method

        return grp_df


    def create_spkr_df(self):
        """
        ...
        :return: dataframe 
        """ 
        spkr_df = self.extract_spkr_ps_feats()
        spkr_df = pd.concat([spkr_df, self.extract_spkr_coord_feats()], axis=1)
        # 3rd call here adding model predictions for grp sat -- just one method

        return spkr_df


    def extract_grp_ps_feats(self):
        """
        ...
        for conversation in self.corpus.iter_conversations():
            ...
            politeness_data = self.ps.summarize(self.corpus, query)
            df = pd.concat([df, politeness_data.to_frame().T], ignore_index=True)
        
        # df.index = group_list
        df.insert(loc=0, column='Group_ID', value=group_list)

        return df


    def extract_grp_coord_feats(self, spkr_df):
        """
        ...
        for conversation in self.corpus.iter_conversations():
            ...
            for feat in coord_feats_cols:
                ...
                # for spkr in spkr_df.index:
                for spkr in spkr_df['Speaker_ID']:
                    if spkr in conversation.get_speaker_ids():
                        coord_score += spkr_df.loc[spkr][feat]
                        coord_count += 1

                total_coord_score = self.average(coord_score, coord_count)
    ...
    def extract_spkr_ps_feats(self):
        """
        ...
        df.insert(loc=0, column='Speaker_ID', value=speaker_list)

        return df


    def extract_spkr_coord_feats(self):
        """
        ...
        for speaker in self.corpus.iter_speakers():
            ...
            for feat in coord_feats_cols:
                coord_score = self.calc_feat_avg(speaker, feat)  
    ...
    def calc_feat_avg(self, speaker, feat):
        """
        ...
        coord_score = self.average(coord_score, coord_count)

        return coord_score


    def average(self, score, count):
        """
