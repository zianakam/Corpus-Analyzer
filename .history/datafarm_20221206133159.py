...
def pre_process(corpus):
    ...
    corpus = ps_parser.transform(corpus)
    corpus = ps.transform(corpus, markers=True) 

    # Coordination
    coord.fit_transform(corpus)
    coord.summarize(corpus)

    return corpus

    
def create_grp_df(corpus):
    """
    ...
    :return: dataframe 
    """ 
    grp_df = extract_grp_ps_feats(corpus)
    # grp_df = pd.concat([grp_df, extract_grp_coord_feats(corpus, spkr_df)], axis=1)

    print(grp_df)
    return grp_df


# run spkr_coord FIRST once and keep df for later use for grp_coord graph generation
# cuz need to get avg spkr coord scores and use that to gen avg group coord scores
def create_spkr_df(corpus):
    """
...
def extract_grp_coord_feats(corpus):
    df = pd.DataFrame()
...
def calc_feat_avg(speaker, feat):
    """
    Calculate the average of a particular feature coordination score 
...
# if not used again, can just lump in with calc_feat_avg function
