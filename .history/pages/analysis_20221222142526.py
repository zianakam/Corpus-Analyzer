...
#@dash.callback(
    ...
    #Input(component_id='user-input', component_property='modified_timestamp'),
    #State(component_id='user-input', component_property='data'),
...
def configure_graph(value):
    if value is 'group-graph':
        df = pd.read_csv("ugi-gap-grp-feats.csv")
        bar_graph = px.bar(
                    df, x='Group', y='Group_Sat',
                )
    elif value is 'spkr-graph':
        df = pd.read_csv("ugi-gap-gen-spkr-feats.csv")
        bar_graph = px.bar(
                    df, x='article', y='quant', # temp graph for functionality purposes
                )
