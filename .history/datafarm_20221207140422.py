...
class DataFrame:
    ...
    parent_dir = "convokitdatasets" # will i be using this elsewhere? if not place directly in function
    coord_feats_cols = ['article', 'auxverb', 'conj', 'adverb', 'ppron', 'ipron', 'preps', 'quant']

    def __init__(self, corpus_name):
        pre_corpus = self.set_corpus(corpus_name)
        self.corpus = self.pre_process(pre_corpus)
    ...
    def create_grp_df(self, spkr_df):
        ...
        :return: dataframe 
        """ 
        grp_df = self.extract_grp_ps_feats(self.corpus)
        grp_df = pd.concat([grp_df, self.extract_grp_coord_feats(self.corpus, spkr_df)], axis=1)
    ...
    def create_spkr_df(self):
        ...
        :return: dataframe 
        """ 
        spkr_df = self.extract_spkr_ps_feats(self.corpus)
        spkr_df = pd.concat([spkr_df, self.extract_spkr_coord_feats(self.corpus)], axis=1)
